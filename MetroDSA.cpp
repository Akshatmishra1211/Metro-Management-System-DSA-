...
vector<string> findShortestPath(const string& start, const string& destination)     // dijastra algo
{
    ...
    vector<string> path;
    if (dist[destination] == numeric_limits<double>::infinity())   // if still the dist is infinity return empty vector
    {
        return path;
    }
...
void nonLoggedInMenu()     // for guests
{
    ...
    for (auto& station : metroGraph)
    {
        cout << station.first << ", ";       // all station names
    }
    cout << "\b\b  " << endl;
    ...
    transform(start.begin(), start.end(), start.begin(), ::toupper);         // to make it case insensitive
    transform(destination.begin(), destination.end(), destination.begin(), ::toupper);

    if (metroGraph.find(start) == metroGraph.end() || metroGraph.find(destination) == metroGraph.end())   // checks if these station exists
    {
...
bool isArticulationPoint(const string& station)
{
    ...
    int originalComponents = 0;     // count number of connected components
    ...
    while (true)
    {
        ...
        switch (choice)
        {
            ...
            case 5:
                ...
                {
                    map<string, map<string, double>> flowGraph;   // later residual graph
                    for (auto& station : metroGraph)
                    {
                        for (auto& neighbor : station.second)
                        {
